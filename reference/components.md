...
- **Mechanism diagrams** `svg.mech` — every `rect/line/path/circle/polyline` draws with `stroke-dashoffset` from `--len` (set by `measureDraw()`), delayed by `calc(.7s + var(--lv) * .15s)` where `--lv` is the element's level; `.fill` rects fade their fill after the stroke; `.mark` arrowheads pop with the snappy spring (wrap them in a positioned `<g>`); `text` fades at `1.2s + lv × .15s`. Org chart: levels 0–8 (nodes, elbows, dimension lines, caption). Loop: three arcs ending 5° before the next station, station squares, tangent arrowheads, an `.orbit` group rotating about the centre (`ring-turn 16s`) with a dot as the ambient, gated by `.slide.visible`.
